...
def constructBayesNet(gameState: hunters.GameState):
    ...
    obsDistances = list(range(maxManhattanDist + MAX_NOISE + 1))
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for eliminationVariable in eliminationOrder:
            ...
            if len(joinedFactor.unconditionedVariables()) == 1:
                ...
                resultFactor = eliminate(joinedFactor, eliminationVariable)
                currentFactorsList.append(resultFactor)
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        conditionedDist = busters.getObservationProbability(noisyDistance, trueDistance)
        return conditionedDist
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            likelihood = self.getObservationProb(observation, pacmanPos, pos, jailPos)
            self.beliefs[pos] = self.beliefs[pos] * likelihood
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
        ...
        for oldPos in self.allPositions:
            ...
            if oldProb > 0:
                ...
                for newPos, transProb in newPosDist.items():
                    newBeliefs[newPos] += oldProb * transProb
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            pos = self.legalPositions[i % len(self.legalPositions)]
